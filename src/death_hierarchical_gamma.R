...
# data over death types.
jags <- jags.model(file.path('jags', 'death_hierarchical_gamma.bug'),
                   data = list('score' = scores$score,
                               'death' = as.numeric(scores$death),
                               'N' = N,
                               'K' = K),
                   n.chains = 4,
                   n.adapt = 1000)
...
p <- ggplot(subset(credible.intervals, Parameter %in% c('Mean', 'Standard Deviation')),
  ...
  coord_flip() +
  ylab('') +
  opts(title = 'Estimated score distribution after dying in Canabalt because of...') +
  xlab('') +
